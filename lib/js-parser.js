...
const indexOfFirstUnnestedMatch = (targetSeq, str) => { // Note: targetSeq cannot be any of [, ], {, }, ', ", :
...
Parser.identifyTokenType = function(token) {
  ...
  if (token.match(operatorMatcher) && indexOfFirstUnnestedMatch(token.match(operatorMatcher)[0], token) !== null) { // If the token includes an un-nested function call or operator
    return 'expression';
...
const operatorPrecedenceTiers = [ // List of all permitted operators, tiered by order of operation
  // Function calls come first
  { operators: ['f()'], matcher: functionCallMatcher, capturingMatcher: new RegExp(`(${referenceMatcher.source})?\\((.*)\\)`) },
  // Parentheses come next, before any other normal operator
  { operators: ['()'], matcher: /\(.*\)/, capturingMatcher: new RegExp(`\\(([^\\(]*?)\\)`)},
  // NOT operations are highest of logical operators, and even come before normal arithmatic and comparison operators.
  { operators: ['!'], matcher: /\![^\=]/, capturingMatcher: new RegExp(`\\!(${operandMatcher.source})`)},
  ...
  { operators: ['&&'], matcher: /\&\&/ }, // AND is above OR but below NOT (and all other operators)
  { operators: ['||'], matcher: /\|\|/ }  // OR is the lowest precedence
];
operatorPrecedenceTiers.forEach(tier => {  // Build capturing matchers for any tiers that didn't need special capturing matchers to be defined.
  tier.capturingMatcher = tier.capturingMatcher || new RegExp(`(${operandMatcher.source})\\s*(${tier.matcher.source})\\s*(${operandMatcher.source})`);
});
...
const parseExpressionToken = (token = "", context = {}) => {
  ...
  operatorPrecedenceTiers.forEach(tier => {
    while (tokenClone.match(tier.matcher)) {
      if (tier.operators[0] === "f()") {
        tokenClone = tokenClone.replace(tier.capturingMatcher, (_, functionNameToken, functionArgsToken) => {
          const func = Parser.parseToken(functionNameToken, context);
          const result = func(...Parser.parseToken(`[${functionArgsToken}]`, context));
          return (typeof result === 'string') ? `\"${result}\"` : result;
        });
      } else if (tier.operators[0] === "()") {
        tokenClone = tokenClone.replace(tier.capturingMatcher, (_, operandToken) => Parser.parseToken(operandToken, context));
      } else if (tier.operators[0] === '!') {
        tokenClone = tokenClone.replace(tier.capturingMatcher, (_, operandToken) => evaluateOperation('!', [Parser.parseToken(operandToken, context)]));
      } else {                          // All other operators
        tokenClone = tokenClone.replace(tier.capturingMatcher, function (_, operandOneToken, operationToken, operandTwoToken){
          let operands = [operandOneToken, operandTwoToken].map(t => Parser.parseToken(t, context));
          let result = evaluateOperation(operationToken, operands);
          // If the result of the expression is a string, e.g. when doing string concatenation, wrap it in quotes before re-inserting it.
          if (operationToken === '+' && (typeof operands[0] !== 'number' || typeof operands[1] !== 'number')) result = `\"${result}\"`;
          return result;
        });
      }
    }
