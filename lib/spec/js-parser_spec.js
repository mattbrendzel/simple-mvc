module.exports = function() {
  ...
  describe('JavaScript Parser', function() {
    ...
    describe('.identifyTokenType', function() {
      context('when the token passed in is an expression', function () {
        const exampleTokens = [
          "1 + 1",
          "3 * 5",
          "(9 - 1) * 2",
          ...
          "1 < 2 || 3 > 4",
          "'hello' + ' ' + 'world'",
          // "[1, 2, 3].length" // This is an expression, but the parser will need to be modified in order to handle it properly.
        ];
        it('returns the string "expression"', function() {
          exampleTokens.forEach(token => assert.equal('expression', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is a string', function () {
        const exampleTokens = [
          "'test'",
          "'hello []'",
          "'{ example }'",
        ...
        it('returns the string "string"', function() {
          exampleTokens.forEach(token => assert.equal('string', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is an array', function () {
        const exampleTokens = [
          "[1, 2, 3]",
        ...
        it('returns the string "array"', function() {
          exampleTokens.forEach(token => assert.equal('array', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is an object', function () {
        const exampleTokens = [
          "{a: 1, b: 2, c: 3}",
          "{name: 'Bob', tools: ['hammer', 'screwdriver']}",
          "{packageName: 'packageOne', details: { weightKg: 5, widthCm: 30, lengthCm: 30, heightCm: 15}}"
        ];
        it('returns the string "object"', function() {
          exampleTokens.forEach(token => assert.equal('object', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is a k-v pair', function () {
        const exampleTokens = [
          "a: 1",
          "tools: ['hammer', 'screwdriver']",
          "details: { weightKg: 5, widthCm: 30, lengthCm: 30, heightCm: 15}"
        ];
        it('returns the string "kvpair"', function() {
          exampleTokens.forEach(token => assert.equal('kvpair', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is a null value', function () {
        const exampleTokens = [
          "null",
          'null'
        ];
        it('returns the string "nullval"', function() {
          exampleTokens.forEach(token => assert.equal('nullval', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is a boolean value', function () {
        const exampleTokens = [
          "true",
          'false'
        ];
        it('returns the string "boolean"', function() {
          exampleTokens.forEach(token => assert.equal('boolean', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is a reference', function () {
        const exampleTokens = [
          'a.b',
          '[1, 2, 3].test',
          '["test"].length',
        ...
        it('returns the string "reference"', function() {
          exampleTokens.forEach(token => assert.equal('reference', Parser.identifyTokenType(token)));
        });
      });
      context('when the token passed in is a number value', function () {
        const exampleTokens = Array.from({length: 40}, () => (Math.floor(Math.random()*40)*0.25).toString());
        it('returns the string "number"', function() {
          exampleTokens.forEach(token => assert.equal('number', Parser.identifyTokenType(token)));
        });
      });
    });
    ...
    describe('.extractNextToken', function() {
      ...
      context('When the next token is a reference', function() {
        const exampleStrings = [
        ...
        it('specifies the token type as "reference"', function() {
          assert.true(results.every(result => result.tokenType === 'reference'));
        });
      });
    });
    ...
    describe('.parseToken', function() {
      ...
      context('when the token is of type "reference"', function() {
        const exampleContext = {a: 1, b: 2, c: {d: 4, e: { f: 9 }}};
        context('when the root of the token is a variable name', function() {
        ...
        context('when the root of the token is a string or array', function() {
          const exampleTokens = [
            '[1, 2, 5, 7].length',
            '"test string".length'
          ];
          ...
          it("returns the value of that string or array's property", function() {
      ...
      context('when the token is of type "expression"', function() {
        context('when the expression contains no parentheses, references, or function calls', function() {
          context('when the expression contains only one operator', function() {
            ...
            context('when the operation is a string concatenation', function() { // special case of `+`
              const exampleTokens = [
                '"string1" + "string2"',
                '"hello " + "world"',
                "'HELLO' + ' WORLD'",
              ...
              it('performs string concatenation', function() {
                exampleTokens.forEach((token, i) => {
                  assert.equal(expectedResults[i], Parser.parseToken(token, exampleContext));
                });
              });
          ...
          context('when the expression contains multiple operators', function () {
            ...
            context('when all operations are string concatenation', function() {
              const exampleTokens = [
                '"hello" + " " + "world"',
                '"heckin\' " + "good " + "time"'
              ];
              ...
              it('concatenates the arguments as strings', function() {
                exampleTokens.forEach((token, i) => {
                  assert.equal(expectedResults[i], Parser.parseToken(token, exampleContext));
                });
              });
        ...
        context('when the expression contains references', function() {
          const exampleTokens = [
            '(true || false) && true',
            'a + 30 > (20 + c)',
            'a * (c + 1)',
            '(a < c || 9 > 1) && b === "s"',
            'c * (4 + 1) > 19 || 0 === 1',
            'c * (3 - 1) >= a + 4 || c * (3 - 1) > 18 - 4',
            '(c * 3) - 1 >= a + 4 || (c * 3) - 1 > 18 - 4',
            '(c > 1 && a < (a + 15) || (23 + c) * 2 > 9) && 2 > 1'
          ];
          const exampleContext = { a: 10, b: 's', c: 5, d: { e: 55 } };
          const expectedResults = [
            true,  // '(true || false) && true',
            true,  // '10 + 30 > (20 + 5)',
            60,    // '10 * (5 + 1)',
          ...
          it('evaluates the entire expression, replacing references with their associated values', function() {
            exampleTokens.forEach((token, i) => {
              assert.equal(expectedResults[i], Parser.parseToken(token, exampleContext));
            });
          });
        ...
        context('when the expression contains function calls', function() {
          ...
          it('evaluates the entire expression, replacing each function call with its resulting value', function() {
            exampleTokens.forEach((token, i) => {
              assert.equal(expectedResults[i], Parser.parseToken(token, exampleContext));
            });
          });
