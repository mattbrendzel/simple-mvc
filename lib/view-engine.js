...
const renderViewFor = function(controller) {
  ...
  return new Promise(function(fulfill, reject){ // Load layout
    fs.readFile(`${root}/config/layouts/${layoutName}.xml`, 'utf8', function(err, data){
  ...
  .then(function(layoutData){
    ...
    return new Promise(function(fulfill, reject){
      fs.readFile(`${root}/app/${controller.resource}/views/${controller.params.action}.xml`, 'utf8', function(err, data){
        err? reject(err) : fulfill(data);
      });
    });
  })
