...
const render = function(str, context) {
  ...
  var partialMatcher = /(<\$>\S*\s\$>)/;
	...
	.map(function(segment){  // handle inline helpers and raw interpolation
      ...
      const operands = CodeParser.parseToken(`[${tokens.slice(1)}]`, context);
      return helpers.inline[tokens[0]](operands, context);
		}
    else {						               // otherwise, parse the value
      return CodeParser.parseToken(tokens[0], context).toString();
		}
	})
	.join('');
};
