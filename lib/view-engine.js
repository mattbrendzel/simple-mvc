...
const parseOperands = function(str, context) {
  // For parsing a string of operands, which may include collections
  // TODO: Build this! Needs to be able to effectively do evaluation, but
  // without using eval().
  return str.split(/\,\s*/);
};
...
const render = function(str, context) {
  ...
  var partialMatcher = /(<\$>\S*\s\$>)/;
	...
	.map(function(segment){  // handle block helpers
		...
		var operands = parseOperands(tokens[2]);
...
const helpers = require(`${root}/lib/view-helpers.js`)(render, parseValue); // Load default helpers
