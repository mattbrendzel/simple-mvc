...
const parseExpressionToken = (token = "", context = {}) => {
  ...
  operatorPrecedenceTiers.forEach(tier => {
    let operatorIsLogicalNot = tier.operators[0] === '!';
    let expressionMatcherWithCapture = new RegExp(
      operatorIsLogicalNot
        ? `\\!(${operandMatcher.source})` // Logical NOT is a unary operator. Since there's only one unary operator, we don't need to collect the type of operation.
        : `(${operandMatcher.source})\\s*(${tier.matcher.source})\\s*(${operandMatcher.source})`
    )

    while (tokenClone.match(tier.matcher)) {
      if (operatorIsLogicalNot) {
