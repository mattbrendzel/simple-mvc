...
const parseExpressionToken = (token = "", context = {}) => {
  ...
  operatorPrecedenceTiers.forEach(operators => {
    ...
    if (operators.join('') ===  '!') { // ! only has one operand, so use a different expression matcher
      expressionMatcherWithCapture = new RegExp(`\\!(${operandMatcher.source})`);
    } else {
      expressionMatcherWithCapture = new RegExp(`(${operandMatcher.source})\\s*(${operatorsMatcher.source})\\s*(${operandMatcher.source})`);
    }
