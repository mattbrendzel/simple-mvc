...
const operatorPrecedenceTiers = [ // List of all permitted operators, tiered by order of operation
  { operators: ['f()'], matcher: functionCallMatcher }, // Function calls come first
  { operators: ['()'], matcher: /\(.*\)/ }, // Parentheses come next, before any other normal operator
...
const parseExpressionToken = (token = "", context = {}) => {
  ...
  operatorPrecedenceTiers.forEach(tier => {
    ...
    if (tier.operators[0] === 'f()') {
      expressionMatcherWithCapture =  new RegExp(`(${referenceMatcher.source})?\\((.*)\\)`);
      // A function call has two arguments that need to be captured:
      // the function name (a reference) and the list of arguments that will be passed to the function itself
    ...
    while (tokenClone.match(tier.matcher)) {
      if (tier.operators[0] === "f()") {
        tokenClone = tokenClone.replace(expressionMatcherWithCapture, (_, functionNameToken, functionArgsToken) => {
