...
const arrayMatcher = /\[.*\]/;
const stringMatcher = /\'.*?\'|\".*?\"/;
const literalMatcher = new RegExp(`${numberMatcher.source}|${booleanMatcher.source}|${nullMatcher.source
...
const referenceMatcher = new RegExp(
  `(` +
    `(?:${literalMatcher.source})(?:\\.${identifierMatcher.source})+|` +       // Literal followed by 1+ subreferences, or
    `(?:${identifierMatcher.source})(?:\\.${identifierMatcher.source})*`+      // An identifier, followed by any number of sub-references
  `)`
);
...
const parseVariableToken = (token = "", context = {}) => { // "x.y"
  return evaluateReferenceChain(context, token.split('.'));
};
...
const parseExpressionToken = (token = "", context = {}) => {
  ...
  // Evaluate Functions and Other References
    ...
    while (tokenClone.match(tier.matcher)) {
      if (tier.operators[0] === "()") {
        tokenClone = tokenClone.replace(expressionMatcherWithCapture, (_, operandToken) => {
          return Parser.parseToken(operandToken);
        });
      } else if (tier.operators[0] === '!') {
        tokenClone = tokenClone.replace(expressionMatcherWithCapture, (_, operandToken) => {
          return evaluateOperation('!', [Parser.parseToken(operandToken)]).toString();
        });
      } else {                          // All other operators
        tokenClone = tokenClone.replace(expressionMatcherWithCapture, (_, operandOneToken, operationToken, operandTwoToken) => {
