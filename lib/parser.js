...
const operatorMatcher = new RegExp(arithmaticOperatorMatcher.source + '|' + comparisonOperatorMatcher.source + '|' + logicalOperatorMatcher.source);
// Operands
...
const functionCallMatcher = new RegExp('(' + varMatcher.source + ')?\\((.*)\\)');
const kvpairMatcher = new RegExp('(' + identifierMatcher.source + ')\s*\:\s*(.+)');
// Expressions
const arithmaticExpressionMatcher = new RegExp('(\\w+)\\s*(' + arithmaticOperatorMatcher.source + ')\\s*(\\w+)');
const comparisonExpressionMatcher = new RegExp('(\\w+)\\s*(' + comparisonOperatorMatcher.source + ')\\s*(\\w+)');
const logicalExpressionMatcher = new RegExp('(\\w+)?\\s*(' + logicalOperatorMatcher.source + ')\\s*(\\w+)');
...
Parser.extractNextToken = function(str, separator = ',') {
  ...
  let tokenType;
  if (str.match(operatorMatcher) && indexOfFirstUnnestedMatch(str.match(operatorMatcher)[0], str) !== null) { // If an un-nested operator exists in the token; includes functions
...
const parseArrayToken = (token = "", context = {}) => {
  ...
  while (parentTokenContent.length > 0) {
    ...
    arr.push(Parser.parseToken(extraction.tokenType, extraction.token, context)); // Recursively parse each array element and add the parsed value to the memo array.
...
const parseObjectToken = (token = "", context = {}) => {
  ...
  while (parentTokenContent.length > 0) {
    ...
    obj[keyToken] = Parser.parseToken(valExtraction.tokenType, valExtraction.token, context); // Update the memo object
    parentTokenContent = extraction.remainingText; // Update the source text to exclude extracted tokens
  }
  return obj;
};
