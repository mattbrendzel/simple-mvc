...
const parseExpressionToken = (token = "", context = {}) => {
  // Function Invocation/Parentheses => Arithmatic => Comparison => Logical
  let tokenClone = token.slice(0);

  // Handle function invocation / parens
  // Handle artithmatic operators
  if (tokenClone.match(arithmaticOperatorMatcher)) {
    console.log("arithmatic", tokenClone)
    tokenClone = tokenClone.replace(arithmaticExpressionMatcherWithCapture, function(_, operandOneToken, operationToken, operandTwoToken) {
      console.log(
        "operation:", operationToken,
        "operandOneToken", operandOneToken,
        "operandTwoToken", operandTwoToken
      );
      const operands = [Parser.parseToken(operandOneToken), Parser.parseToken(operandTwoToken)];
  ...
  if (tokenClone.match(comparisonOperatorMatcher)) {
    console.log("comparison", tokenClone)
    tokenClone = tokenClone.replace(comparisonExpressionMatcherWithCapture, function(_, operandOneToken, operationToken, operandTwoToken) {
      console.log(
        "operation:", operationToken,
        "operandOneToken", operandOneToken,
        "operandTwoToken", operandTwoToken
  ...
  if (tokenClone.match(logicalOperatorMatcher)) {
    console.log("logical", tokenClone);
    tokenClone = tokenClone.replace(logicalExpressionMatcherWithCapture, function(_, operandOneToken, operationToken, operandTwoToken) {
      console.log(
        "operation:", operationToken,
        "operandOneToken", operandOneToken,
        "operandTwoToken", operandTwoToken
      );
      const operands = [operandOneToken && Parser.parseToken(operandOneToken), Parser.parseToken(operandTwoToken)]; // operandOneToken will be null in the case of !
      switch(operationToken) {
        case '||':
          return operands[0] || operands[1];
        case '&&':
  ...
  return Parser.parseToken(tokenClone);
};
