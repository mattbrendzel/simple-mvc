...
const findFileWithin = function(path, targetFilename) {
  ...
  return filepathExists(`${path}/${targetFilename}`) // If the file is in the root of the target directory, return it.
  .catch(function(err){                              // Otherwise do the following:
...
FileLoader.loadPartial = function(partialName) {
  return loadFile(`app/resources/shared/_${partialName}.xml`)
  .catch(() => FileLoader.raiseError(new Error("File Error: Partial template not found.")));
}

FileLoader.loadSharedPartials = function() {
  return getDirectoryContents("app/resources/shared").catch(e => []) // If it fails, return an empty array.
  .then(entities => { // Only keep XML files
    return entities.filter(entity =>  entity.isFile() && entity.name.match(/\.xml$/))
    .map(entity => entity.name.slice(1, -4)); // Return the filenames, minus the underscores and the .xml extensions
  })
  .then(partialNames => {
    const partials = {};
    return Promise.all(partialNames.map(name => {
      return FileLoader.loadPartial(name).then(partial => partials[name] = partial);
    })).then(() => partials);
  })
};
