  ...
  .catch(function(err){
    ...
    .then(function(entities){ // Determine which things in the directory are subdirectories
      ...
      if (subdirectories.length === 0) {
        return Error.new(`Failed - Could not find file ${targetFilename} in ${path}`);
      }
    ...
    .catch(function(){
      // Better error handling here in the future.
      console.log(`Failed - Could not find file ${targetFilename} in ${path}`);
    });
  });
};
...
FileLoader.loadAsset = function(filename){ // Manages asset pipeline
  return findFileWithin("app/assets", filename)
  .then(function(result){
    return result || findFileWithin("lib/assets", filename)
  })
  .then(function(result){
    return result || findFileWithin("public", filename)
  })
  .then(function(result){
    if (!result) { return Promise.reject(new Error("Asset not found.")); }
    console.log(result);
